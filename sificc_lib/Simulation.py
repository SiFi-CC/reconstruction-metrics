...
class Simulation:
    ...
    def iterate_events(self, basket_size=100000, desc='processing root file', 
                       bar_update_size=1000, entry_start=None, entry_stop=None):
        ...
        end = self.num_entries if entry_stop is None else entry_stop
        prog_bar = tqdm(total=end-start, ncols=100, file=sys.stdout, desc=desc)
        bar_step = 0
        for start, end, basket in self.tree.iterate(Event.l_leaves, entrysteps=basket_size, 
                                                    reportentries=True, namedecode='utf-8',
                                                    entrystart=entry_start, entrystop=entry_stop):
            length = end-start
